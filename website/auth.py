...
def register():
    if request.method == 'POST':
        ...
        check_badge = Employee.query.filter_by(
            badge_number=badge_number).first()
        if check_mail:
...
def update_user_notifications():
    user_notifications_relation = NotificationUser.query.filter_by(
        user_id=current_user.id).all()
    notifications = []
    for relation in user_notifications_relation:
        notifications.append(Notification.query.filter_by(
            id=relation.notification_id).first())
